...
export default function ListeningPreviewPage() {
  ...
  const fetchMock = useCallback(async () => {
    try {
      ...
      const parts = [1, 2, 3, 4].map((n) => {
        ...
        return found || { partNumber: n, audioUrl: '', transcript: '', questions: [] };
      });
      setMock({ ...data, parts });
    } catch {
  ...
  const calcScore = () => {
    ...
    mock.parts.forEach((p: any, pi: number) => {
      p.questions.forEach((q: any, qi: number) => {
        total++;
        const key = `${pi}_${qi}`;
        if ((answers[key] || '').trim().toLowerCase() === (q.correctAnswer || '').trim().toLowerCase()) correct++;
      });
    });
  ...
  const totalQuestions = mock.parts.reduce((s: number, p: any) => s + (p.questions?.length || 0), 0);
    ...
    <div className="flex flex-col h-full -m-8">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/listening/${id}/build`} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors">
            <ArrowLeft size={18} />
          </Link>
          <div>
        ...
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/listening/${id}/build`} className="flex items-center gap-2 px-4 py-2 border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
            <Wrench size={16} /> Back to Builder
          </Link>
          {!submitted && (
            <button onClick={() => setSubmitted(true)} className="px-5 py-2 bg-teal-600 text-white rounded-lg text-sm font-semibold hover:bg-teal-700 transition-colors">
              Submit Answers
            </button>
          )}
        </div>
      ...
      {submitted && (
        <div className={`mx-6 mt-4 p-5 rounded-xl border-2 flex flex-col sm:flex-row items-center gap-6 ${bandColor}`}>
          <div className="text-center sm:text-left flex-1">
            <div className="text-lg font-bold">Listening Test Complete! 🎧</div>
            <div className="text-sm mt-0.5 opacity-80">
              {correct} / {total} correct ({Math.round((correct / Math.max(total, 1)) * 100)}%)
            </div>
          </div>
          ...
          <button onClick={() => { setAnswers({}); setSubmitted(false); }} className="px-5 py-2 bg-white bg-opacity-70 rounded-lg text-sm font-semibold border border-current hover:bg-opacity-100 transition-colors">
            Retry
          </button>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden mt-4">
        ...
        <div className="w-48 bg-slate-50 border-r border-slate-200 flex flex-col p-4 gap-2 shrink-0">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-2">Parts</p>
          {mock.parts.map((p: any, idx: number) => {
          ...
          <div className="mt-auto pt-4 border-t border-slate-200">
            <div className="text-xs text-slate-500 px-2">
              <div className="flex justify-between"><span>Answered</span><span className="font-bold">{answeredCount}/{totalQuestions}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-80 xl:w-96 overflow-y-auto p-6 bg-slate-50 flex flex-col gap-4 shrink-0">
            <h3 className="font-bold text-slate-800">Questions – Part {activePart + 1}</h3>
            {(part.questions || []).length === 0 ? (
              <div className="text-center py-8 text-slate-400 text-sm">No questions for this part.</div>
            ) : (
              part.questions.map((q: any, qi: number) => {
                const key = `${activePart}_${qi}`;
                ...
                return (
                  <div key={qi} className={`bg-white rounded-xl border p-4 ${
                    submitted ? isCorrect ? 'border-green-300 bg-green-50' : isWrong ? 'border-red-300 bg-red-50' : 'border-amber-200 bg-amber-50' : 'border-slate-200'
                  }`}>
                    ...
                    {q.questionType === 'multiple_choice' && q.options.length > 0 && (
                      <div className="space-y-2">
                        {q.options.map((opt: string, oi: number) => (
                          <label key={oi} className={`flex items-center gap-2.5 p-2 rounded-lg border cursor-pointer text-sm ${
                            userAnswer === opt ? submitted ? isCorrect ? 'border-green-400 bg-green-100' : 'border-red-400 bg-red-100' : 'border-teal-500 bg-teal-50' : submitted && opt === q.correctAnswer ? 'border-green-400 bg-green-100' : 'border-slate-200 hover:border-teal-300'
                          }`}>
                            <input type="radio" name={`q-${key}`} value={opt} checked={userAnswer === opt} disabled={submitted} onChange={() => setAnswer(key, opt)} className="w-4 h-4" />
                            {opt}
                          </label>
                        ))}
                      </div>
                    ...
                    {q.questionType === 'true_false_not_given' && (
                      <div className="flex gap-2">
                        {['True', 'False', 'Not Given'].map(opt => (
                          <button key={opt} type="button" disabled={submitted} onClick={() => setAnswer(key, opt)}
                            className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-all ${
                              userAnswer === opt ? submitted ? isCorrect ? 'bg-green-500 text-white border-green-500' : 'bg-red-500 text-white border-red-500' : 'bg-teal-600 text-white border-teal-600'
                                : submitted && opt === q.correctAnswer ? 'bg-green-500 text-white border-green-500' : 'border-slate-300 text-slate-600'
                            }`}
                          >{opt}</button>
                        ))}
                      </div>
                    ...
                    {['matching_headings', 'matching_information', 'sentence_completion', 'summary_completion', 'short_answer'].includes(q.questionType) && (
                      <input type="text" value={userAnswer} disabled={submitted} onChange={(e) => setAnswer(key, e.target.value)}
                        className={`w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 ${submitted ? isCorrect ? 'border-green-400 bg-green-50' : 'border-red-400 bg-red-50' : 'border-slate-300'}`}
            ...
            {!submitted && (part.questions || []).length > 0 && (
              <button onClick={() => setSubmitted(true)} className="w-full py-3 bg-teal-600 text-white rounded-xl font-semibold text-sm hover:bg-teal-700 transition-colors mt-2">
                Submit All Answers
              </button>
            )}
          </div>
